...
model_file = '/data2/simingy/model/tvgg16_gate_recurrent_v2_conv3333_gt5g1_gt1_d_lrelu_unroll1_again/best.pkl'
model_name = 'multiple_recurrent_l3' # for now
...
# not sure if the data is shuffled -- might not be a random sample of image which would be bad
#for x, y in itertools.islice(test_loader, 32):
    #x = A.Variable(x.cuda(), volatile=True) # not doing backprop so volatile
    #y = A.Variable(y.cuda(), volatile=True) # this bit taken from train_on_imagenet
#
...
#life_file = open('vgg_pop_sparsenesses.pkl', 'wb')
